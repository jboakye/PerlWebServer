...
sub readRequest{
    #Requete
    my ($listen_sock, $fils, $confs) = @_;
    my $chunk_size = 1024;
    #listen(SERVER, SOMAXCONN) or die "Listen : $!";
    my ($client, $iaddr) = $listen_sock->accept();

    #jbdebug
    print Dumper('---- peer address----', $iaddr);
    ...
    if( @$fils > $confs->{"set"}{"clients"}){
        ...
        if($pidReq == 0){
            ...
            if(validateRequest($headers) != 0)
            {
                ...
                $chemin = projectionsCheck($headers,\@routes,$confs);
